...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False):
    flash_supported_dtype = q.dtype in (torch.float16, torch.bfloat16)
    flash_supported_device = q.is_cuda
    use_flash_kernel = (not compatibility_mode) and flash_supported_dtype and flash_supported_device

    if compatibility_mode or not use_flash_kernel:
...
class ConditionalCrossAttentionBlock(nn.Module):
    ...
    def __init__(self, dim: int, kv_dim: int, num_heads: int, eps: float = 1e-6, pooled_adaln: bool = False):
        ...
        if self.pooled_adaln:
            self.adaln_norm = nn.LayerNorm(dim, eps=eps)
            self.adaln_proj = nn.Linear(kv_dim, 2 * dim)

    def per_frame_pooling(self, y: torch.Tensor, video_grid_size: Tuple[int, int, int]) -> torch.Tensor:
        b, l, c = y.shape
        f, h, w = video_grid_size
        expected = f * h * w
        if l != expected:
            # Fallback: average all tokens then repeat over temporal axis.
            pooled = y.mean(dim=1, keepdim=True)
            return pooled.expand(b, f, c)
        return y.view(b, f, h * w, c).mean(dim=2)

    def adaln(self, x: torch.Tensor, temb: torch.Tensor) -> torch.Tensor:
        shift, scale = self.adaln_proj(temb).chunk(2, dim=-1)
        x = self.adaln_norm(x)
