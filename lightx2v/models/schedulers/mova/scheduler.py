...
class FlowMatchScheduler:
    ...
    def set_timesteps(self, num_inference_steps=100, denoising_strength=1.0, shift=None, dynamic_shift_len=None, device=None):
        ...
        timesteps = self.sigmas * self.num_train_timesteps
        self.timesteps = timesteps.to(device=device)
        if self.train_timesteps is None:
            self.train_timesteps = self.timesteps
            self.train_sigmas = self.sigmas.to(device=device)
    ...
    def step(self, model_output, timestep, sample, to_final=False, **kwargs):
        ...
        sigma_delta = torch.as_tensor(sigma_ - sigma, device=target_device, dtype=target_dtype)
        prev_sample = sample + model_output.to(dtype=target_dtype) * sigma_delta
        return prev_sample.to(dtype=target_dtype)
...
class FlowMatchPairScheduler(FlowMatchScheduler):
    ...
    def timestep_to_sigma(self, timestep: torch.Tensor | float) -> torch.Tensor:
        ...
        t_cur = torch.tensor(t_value, device=self.train_timesteps.device, dtype=self.train_timesteps.dtype)
        idx = torch.argmin((self.train_timesteps - t_cur).abs())
    ...
    def step_from_to(self, model_output: torch.Tensor, timestep_from: torch.Tensor, timestep_to: torch.Tensor | None, sample: torch.Tensor) -> torch.Tensor:
        ...
        target_dtype = sample.dtype
        target_device = sample.device
        sigma_delta = (sigma_to - sigma_from).to(device=target_device, dtype=torch.float32)
        prev_sample = sample.to(dtype=torch.float32) + model_output.to(device=target_device, dtype=torch.float32) * sigma_delta
        return prev_sample.to(dtype=target_dtype)
...
class MovaAudioPairScheduler:

    def __init__(self, pair_scheduler):
        self.pair_scheduler = pair_scheduler

    def __getattr__(self, name):
        # Delegate unknown attributes to paired scheduler so shared
        # runtime fields (e.g. sample_guide_scale, step_index) stay usable.
        return getattr(self.pair_scheduler, name)

    @property
    def noise_pred(self):
        return self.pair_scheduler.audio_noise_pred

    @noise_pred.setter
    def noise_pred(self, value):
        self.pair_scheduler.audio_noise_pred = value

    @noise_pred.deleter
    def noise_pred(self):
        del self.pair_scheduler.audio_noise_pred

    @property
    def latents(self):
        return self.pair_scheduler.audio_latents

    @property
    def timestep_input(self):
        return self.pair_scheduler.audio_timestep_input

    @property
    def sample_guide_scale(self):
        return self.pair_scheduler.sample_guide_scale

    @sample_guide_scale.setter
    def sample_guide_scale(self, value):
        self.pair_scheduler.sample_guide_scale = value
...
class MovaPairScheduler(BaseScheduler):
    ...
    def total_steps(self):
        return self.paired_timesteps.shape[0]

    def prepare(self, seed, latent_shape, audio_latent_shape):
        self.scheduler.set_timesteps(self.infer_steps, device=AI_DEVICE)
        # visual_shift = float(
        #     self.config.get(
        #         "visual_shift",
        #         self.config.get("sample_shift", self.config.get("sigma_shift", 5.0)),
        #     )
        # )
        # audio_shift = float(self.config.get("audio_shift", visual_shift))
        # self.scheduler.set_pair_postprocess_by_name(
        #     "dual_sigma_shift",
        #     visual_shift=visual_shift,
        #     audio_shift=audio_shift,
        # )
        self.audio_scheduler = self.scheduler
        self.paired_timesteps = self.scheduler.get_pairs()

        self.prepare_latents(seed, latent_shape, audio_latent_shape, dtype=GET_DTYPE())
        self.step_index = 0

    def step_pre(self, step_index):
        self.step_index = step_index
        self.timestep_input = torch.stack([self.paired_timesteps[self.step_index, 0]])
        self.audio_timestep_input = torch.stack([self.paired_timesteps[self.step_index, 1]])

    def step_post(self):
        next_timestep = self.paired_timesteps[self.step_index + 1, 0] if self.step_index + 1 < self.total_steps else None
        current_timestamp = self.paired_timesteps[self.step_index, 0]
        next_audio_timestep = self.paired_timesteps[self.step_index + 1, 1] if self.step_index + 1 < self.total_steps else None
        current_audio_timestamp = self.paired_timesteps[self.step_index, 1]

        self.latents = self.scheduler.step_from_to(
            self.noise_pred,
            current_timestamp,
            next_timestep,
            self.latents,
        )
        self.audio_latents = self.audio_scheduler.step_from_to(
            self.audio_noise_pred,
            current_audio_timestamp,
            next_audio_timestep,
            self.audio_latents,
        )

    def prepare_latents(self, seed, latent_shape, audio_latent_shape, dtype=torch.float32):
        ...
        self.audio_generator = torch.Generator(device=AI_DEVICE).manual_seed(seed)
        self.audio_latents = torch.randn(
            audio_latent_shape[0],
            audio_latent_shape[1],
            dtype=dtype,
            device=AI_DEVICE,
