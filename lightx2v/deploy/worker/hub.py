...
class BaseWorker:
    ...
    def set_inputs(self, params):
        ...
        if "target_shape" in self.input_info.__dataclass_fields__:
            if self.is_t2v_task():
                height, width = params.get("custom_shape", [])
                if (width <= 1280 and height <= 720 and width >= 256 and height >= 256) or (
                        width <= 720 and height <= 1280 and width >= 256 and height >= 256):
